...
function Invoke-RemoteUpdate {
    ...
    # Step 2: Run deploy.sh update with branch environment variable
...
function Main {
    ...
    # Initialize script-level variables with default values
    $script:Server = ""
    $script:Port = "22"
    $script:User = ""
    $script:Password = ""
    ...
    # Apply config values if parameters not provided (config file takes priority over script defaults)
    if (-not $Server) { $Server = $script:Server }
    if (-not $Port) { $Port = $script:Port }
    ...
    if (-not $Server -or -not $User) {
        Read-ServerConfig
        # After prompting, use the interactive values
        if ($script:Server) { $Server = $script:Server }
